// Package dashtest provides an in-process fake Ceph Dashboard serving
// Reef-shaped JSON for tests. It exists so provider package tests and sync
// wiring tests run the same HTTP contract without a real cluster.
package dashtest
...
func New(t *testing.T, mode Mode) *Dashboard {
	...
	mux.HandleFunc("GET /api/osd", guard(dashboard.handleOSDs))
	...
	mux.HandleFunc("GET /api/host/", guard(dashboard.handleHostItem))
	dashboard.server = httptest.NewServer(mux)
	t.Cleanup(dashboard.server.Close)
	return dashboard
}
