...
Evaluation semantics: a firing alert with no row (or a row whose linked Case is closed) creates a new Case plus a `case_detected` Timeline Event (carrying normalized OSD/Host context from the current read model when the alert targets a resolvable OSD) and the dedup row in one transaction; a firing alert matching an open Case refreshes the dedup row's sighting fields (`last_seen_at`, `alert_name`, `cluster_label`) without creating anything. A `resolved` observation marks the row `resolved`; a `pending` observation refreshes `last_seen_at` and leaves the state unchanged. Neither closes the Case: auto-close is a policy decision and no policy exists yet. Reopen after a manual close creates a new Case and re-points the row. Concurrent evaluations serialize on a transaction-scoped advisory lock so the check-then-act on fingerprints cannot race; the whole losing transaction rolls back.
...
- After a reopen, the superseded closed Case no longer carries the detection linkage; reads surface it on the active Case only.
